defmodule Test do
  ...
  def recv_regex([new | rest], data) do
    if Regex.match?(~r/.*\r\n$/s, new),
      do: data <> new,
      else: recv_regex(rest, data <> new)
  end

  def recv_match(), do: load_file() |> recv_match("")

  def recv_match([new | rest], data) do
    new
    |> reverse()
    |> handle_recv(rest, data)
  end

  defp handle_recv("\n\r" <> reversed = _new, _rest, data),
    do: data <> reverse(reversed)

  defp handle_recv(new, rest, data),
    do: recv_match(rest, data <> new)
